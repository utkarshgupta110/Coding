...
void insertDirectedEdge(int graph[v][v], int start, int end) {
...
void insertUndirectedEdge(int graph[v][v], int start, int end) {
    // Check if the vertices are within bounds
    /*if (start < 0 || start >= v || end < 0 || end >= v) {
    printf("Invalid vertices: %d and %d. Vertices must be between 0 and %d.\n", start, end, v - 1);
    return;
    ...
    graph[start][end] = 1;
    graph[end][start] = 1; // Add edge from start to end (for undirected graph)
    printf("Edge inserted between %d and %d.\n", start, end);
}
...
int main(){
    ...
    insertUndirectedEdge(graph, 0, 1);
    insertUndirectedEdge(graph, 4, 5);
    insertUndirectedEdge(graph, 2, 3);
    insertDirectedEdge(graph, 1, 2);
    insertDirectedEdge(graph, 4, 2);
    insertDirectedEdge(graph, 4, 0);
    insertDirectedEdge(graph, 5, 3);
